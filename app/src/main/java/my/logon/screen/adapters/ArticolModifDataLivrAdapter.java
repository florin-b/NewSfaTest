...
public class ArticolModifDataLivrAdapter extends BaseAdapter implements OperatiiArticolListener {
	...
	public ArticolModifDataLivrAdapter(Context context, List<ArticolComanda> listArticole, List<BeanConditiiArticole> listConditii, BeanComandaCreata comanda) {
	...
	public static class ViewHolder {
		TextView textNrCrt, textNumeArt, textCodArt, textCantArt, textUmArt, textPretArt, textMonedaArt, textDepozit, textStatusArt, textProcRed,
				textCantConditie, textUmConditie, textPretConditie, textMonedaConditie, textConditie, textObsArt, textAcZcDeLivrat, umAcZcDeLivrat, textAcZcLivrat, umAcZcLivrat;

		LinearLayout layoutAcZcDeLivrat, layoutAcZcLivrat;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(context).inflate(R.layout.articol_modif_data_livrare, parent, false);
			...
			viewHolder.textAcZcDeLivrat = (TextView) convertView.findViewById(R.id.textAcZcDeLivrat);
			viewHolder.umAcZcDeLivrat = (TextView) convertView.findViewById(R.id.umAcZcDeLivrat);
			viewHolder.layoutAcZcDeLivrat = (LinearLayout) convertView.findViewById(R.id.layoutAcZcDeLivrat);

			viewHolder.textAcZcLivrat = (TextView) convertView.findViewById(R.id.textAcZcLivrat);
			viewHolder.umAcZcLivrat = (TextView) convertView.findViewById(R.id.umAcZcLivrat);
			viewHolder.layoutAcZcLivrat = (LinearLayout) convertView.findViewById(R.id.layoutAcZcLivrat);
		...
		viewHolder.layoutAcZcDeLivrat.setVisibility(View.GONE);
		viewHolder.layoutAcZcLivrat.setVisibility(View.GONE);
		if (comanda.isComandaACZC() && !articol.getCodArticol().startsWith("30")) {
			viewHolder.layoutAcZcDeLivrat.setVisibility(View.VISIBLE);
			viewHolder.textAcZcDeLivrat.setText(nf3.format(articol.getAczcDeLivrat()));
			viewHolder.umAcZcDeLivrat.setText(articol.getUm());

			viewHolder.layoutAcZcLivrat.setVisibility(View.VISIBLE);
			viewHolder.textAcZcLivrat.setText(nf3.format(articol.getAczcLivrat()));
			viewHolder.umAcZcLivrat.setText(articol.getUm());
